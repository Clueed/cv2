---
title: Mark — Education
category: references
tags: [cv, education]
relationships:
  - target: "[[entities/mark-vavulov]]"
    type: derived_from
sources:
  - resume-context.md
summary: Mark's three educational stops — CBS Cologne (Int'l Business B.A., 1.5 GPA), JCU Singapore (semester abroad, pivot to CS), TU Berlin (CS B.Sc., paused).
provenance:
  extracted: 0.95
  inferred: 0.05
  ambiguous: 0.0
base_confidence: 0.55
lifecycle: draft
lifecycle_changed: 2026-05-24
tier: supporting
created: 2026-05-24T00:00:00Z
updated: 2026-05-24T00:00:00Z
---

# Mark — Education

Reference page for CV tailoring: full detail on [[entities/mark-vavulov|Mark]]'s academic history.

## TU Berlin — Computer Science B.Sc. (Oct 2023 – Oct 2024)

- Enrolled after completing the business degree at CBS
- Completed coursework in **advanced mathematics** and **systems programming**
- **Paused** to join Stackgini full-time; studies on hold, not terminated
- Also active in **TU Berlin's startup incubator** (Center of Entrepreneurship)

## James Cook University Singapore — Semester Abroad (Jul – Dec 2022)

- Taken during the CBS program
- Took a **software engineering course**
- Built a **full-stack multiplayer web game** in a SCRUM team
- This experience drove the decision to study CS formally and enroll at TU Berlin after finishing the business degree

## CBS Cologne Business School — International Business B.A. (Oct 2020 – Aug 2023)

- **GPA: 90% (German grade 1.5)**
- **Thesis:** whether DAX companies use R&D capitalization under IFRS to manipulate reported earnings
- Singapore semester abroad taken during this program

## CV tailoring notes

- The IFRS R&D capitalization thesis is the strongest signal of financial-statement literacy at the academic level — useful when a JD touches accounting, audit, or financial reporting.
- TU Berlin's Center of Entrepreneurship affiliation is a niche detail; surface it when a JD signals startup ecosystem or founder-track relevance.
- The JCU Singapore → TU Berlin pivot is the cleanest story for "why CS after business" — a single course converted the trajectory.

## Related

- [[entities/mark-vavulov]]
- [[references/mark-prior-roles]]
